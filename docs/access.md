...
| --- | --- | --- |
| `staging` | `g3q-backend-staging` | `g3q_backend_staging` |
| `main` | production App Service (not provisioned yet) | production DB (not provisioned yet) |

Legacy alias: `g3q-backend` still runs against `g3q_backend_staging` during cutover. Prefer the `-staging` name.
...
- **Azure CLI (`az`)**: Access to Azure App Service (`g3q-backend-staging`), Azure MySQL (`edutor-mysql`), and subscription resources (refer to `docs/g3q-azure.md`).
