/** Gujarat districts + talukas with gu/en/hi labels for auth dropdowns. */
export const GUJARAT_DISTRICTS = [
  {
    "nameGu": "કચ્છ",
    "nameEn": "Kutch",
    "nameHi": "कच्छ",
    "talukas": [
      {
        "nameGu": "ભુજ",
        "nameEn": "Bhuj",
        "nameHi": "भुज"
      },
      {
        "nameGu": "અબડાસા",
        "nameEn": "Abdasa",
        "nameHi": "अबडासा"
      },
      {
        "nameGu": "અંજાર",
        "nameEn": "Anjar",
        "nameHi": "अंजार"
      },
      {
        "nameGu": "ભચાઉ",
        "nameEn": "Bhachau",
        "nameHi": "भचाउ"
      },
      {
        "nameGu": "ગાંધીધામ",
        "nameEn": "Gandhidham",
        "nameHi": "गांधीधाम"
      },
      {
        "nameGu": "લખપત",
        "nameEn": "Lakhpat",
        "nameHi": "लखपत"
      },
      {
        "nameGu": "માંડવી",
        "nameEn": "Mandvi",
        "nameHi": "मांडवी"
      },
      {
        "nameGu": "મુન્દ્રા",
        "nameEn": "Mundra",
        "nameHi": "मुन्द्रा"
      },
      {
        "nameGu": "નખત્રાણા",
        "nameEn": "Nakhatrana",
        "nameHi": "नखत्राणा"
      },
      {
        "nameGu": "રાપર",
        "nameEn": "Rapar",
        "nameHi": "रापर"
      }
    ]
  },
  {
    "nameGu": "બનાસકાંઠા",
    "nameEn": "Banaskantha",
    "nameHi": "बनासकांठा",
    "talukas": [
      {
        "nameGu": "પાલનપુર",
        "nameEn": "Palanpur",
        "nameHi": "पालनपुर"
      },
      {
        "nameGu": "અમીરગઢ",
        "nameEn": "Amirgadh",
        "nameHi": "अमीरगढ"
      },
      {
        "nameGu": "ભાભર",
        "nameEn": "Bhabhar",
        "nameHi": "भाभर"
      },
      {
        "nameGu": "દાંતા",
        "nameEn": "Danta",
        "nameHi": "दांता"
      },
      {
        "nameGu": "દિસા",
        "nameEn": "Deesa",
        "nameHi": "दिसा"
      },
      {
        "nameGu": "દિયોદર",
        "nameEn": "Deodar",
        "nameHi": "दियोदर"
      },
      {
        "nameGu": "ધાનેરા",
        "nameEn": "Dhanera",
        "nameHi": "धानेरा"
      },
      {
        "nameGu": "કાંકરેજ",
        "nameEn": "Kankrej",
        "nameHi": "कांकरेज"
      },
      {
        "nameGu": "વડગામ",
        "nameEn": "Vadgam",
        "nameHi": "वडगाम"
      }
    ]
  },
  {
    "nameGu": "વાવ-થરાદ",
    "nameEn": "Vav-Tharad",
    "nameHi": "वाव-थराद",
    "talukas": [
      {
        "nameGu": "વાવ",
        "nameEn": "Vav",
        "nameHi": "वाव"
      },
      {
        "nameGu": "થરાદ",
        "nameEn": "Tharad",
        "nameHi": "थराद"
      },
      {
        "nameGu": "સુઈગામ",
        "nameEn": "Suigam",
        "nameHi": "सुईगाम"
      },
      {
        "nameGu": "લાખાણી",
        "nameEn": "Lakhani",
        "nameHi": "लाखाणी"
      }
    ]
  },
  {
    "nameGu": "પાટણ",
    "nameEn": "Patan",
    "nameHi": "पाटन",
    "talukas": [
      {
        "nameGu": "પાટણ",
        "nameEn": "Patan",
        "nameHi": "पाटण"
      },
      {
        "nameGu": "ચાણસ્મા",
        "nameEn": "Chanasma",
        "nameHi": "चाणस्मा"
      },
      {
        "nameGu": "હરિજ",
        "nameEn": "Harij",
        "nameHi": "हरिज"
      },
      {
        "nameGu": "રાધનપુર",
        "nameEn": "Radhanpur",
        "nameHi": "राधनपुर"
      },
      {
        "nameGu": "સમી",
        "nameEn": "Sami",
        "nameHi": "समी"
      },
      {
        "nameGu": "સંખેશ્વર",
        "nameEn": "Shankheshwar",
        "nameHi": "संखेश्वर"
      },
      {
        "nameGu": "સાંતલપુર",
        "nameEn": "Santalpur",
        "nameHi": "सांतलपुर"
      },
      {
        "nameGu": "સરસ્વતી",
        "nameEn": "Saraswati",
        "nameHi": "सरस्वती"
      },
      {
        "nameGu": "સિદ્ધપુર",
        "nameEn": "Sidhpur",
        "nameHi": "सिद्धपुर"
      }
    ]
  },
  {
    "nameGu": "મહેસાણા",
    "nameEn": "Mehsana",
    "nameHi": "मेहसाणा",
    "talukas": [
      {
        "nameGu": "મહેસાણા",
        "nameEn": "Mehsana",
        "nameHi": "महेसाणा"
      },
      {
        "nameGu": "બેચરાજી",
        "nameEn": "Becharaji",
        "nameHi": "बेचराजी"
      },
      {
        "nameGu": "જોટાણા",
        "nameEn": "Jotana",
        "nameHi": "जोटाणा"
      },
      {
        "nameGu": "કડી",
        "nameEn": "Kadi",
        "nameHi": "कडी"
      },
      {
        "nameGu": "ખેરાલુ",
        "nameEn": "Kheralu",
        "nameHi": "खेरालु"
      },
      {
        "nameGu": "સતલાસણા",
        "nameEn": "Satlasana",
        "nameHi": "सतलासणा"
      },
      {
        "nameGu": "ઉંઝા",
        "nameEn": "Unjha",
        "nameHi": "उंझा"
      },
      {
        "nameGu": "વડનગર",
        "nameEn": "Vadnagar",
        "nameHi": "वडनगर"
      },
      {
        "nameGu": "વિજાપુર",
        "nameEn": "Vijapur",
        "nameHi": "विजापुर"
      },
      {
        "nameGu": "વિસનગર",
        "nameEn": "Visnagar",
        "nameHi": "विसनगर"
      }
    ]
  },
  {
    "nameGu": "સાબરકાંઠા",
    "nameEn": "Sabarkantha",
    "nameHi": "साबरकांठा",
    "talukas": [
      {
        "nameGu": "હિમતનગર",
        "nameEn": "Himatnagar",
        "nameHi": "हिमतनगर"
      },
      {
        "nameGu": "ઇડર",
        "nameEn": "Idar",
        "nameHi": "इडर"
      },
      {
        "nameGu": "ખેડબ્રહ્મા",
        "nameEn": "Khedbrahma",
        "nameHi": "खेडब्रह्मा"
      },
      {
        "nameGu": "પોશીના",
        "nameEn": "Poshina",
        "nameHi": "पोशीना"
      },
      {
        "nameGu": "પ્રાંતિજ",
        "nameEn": "Prantij",
        "nameHi": "प्रांतिज"
      },
      {
        "nameGu": "તલોદ",
        "nameEn": "Talod",
        "nameHi": "तलोद"
      },
      {
        "nameGu": "વડાલી",
        "nameEn": "Vadali",
        "nameHi": "वडाली"
      },
      {
        "nameGu": "વિજયનગર",
        "nameEn": "Vijaynagar",
        "nameHi": "विजयनगर"
      }
    ]
  },
  {
    "nameGu": "અરવલ્લી",
    "nameEn": "Aravalli",
    "nameHi": "अरावली",
    "talukas": [
      {
        "nameGu": "મોડાસા",
        "nameEn": "Modasa",
        "nameHi": "मोडासा"
      },
      {
        "nameGu": "બાયડ",
        "nameEn": "Bayad",
        "nameHi": "बायड"
      },
      {
        "nameGu": "ભિલોડા",
        "nameEn": "Bhiloda",
        "nameHi": "भिलोडा"
      },
      {
        "nameGu": "ધનસુરા",
        "nameEn": "Dhansura",
        "nameHi": "धनसुरा"
      },
      {
        "nameGu": "માલપુર",
        "nameEn": "Malpur",
        "nameHi": "मालपुर"
      },
      {
        "nameGu": "મેઘરાજ",
        "nameEn": "Meghraj",
        "nameHi": "मेघराज"
      }
    ]
  },
  {
    "nameGu": "ગાંધીનગર",
    "nameEn": "Gandhinagar",
    "nameHi": "गांधीनगर",
    "talukas": [
      {
        "nameGu": "ગાંધીનગર",
        "nameEn": "Gandhinagar",
        "nameHi": "गांधीनगर"
      },
      {
        "nameGu": "દહેગામ",
        "nameEn": "Dehgam",
        "nameHi": "दहेगाम"
      },
      {
        "nameGu": "કલોલ",
        "nameEn": "Kalol",
        "nameHi": "कलोल"
      },
      {
        "nameGu": "માંસા",
        "nameEn": "Mansa",
        "nameHi": "मांसा"
      }
    ]
  },
  {
    "nameGu": "અમદાવાદ",
    "nameEn": "Ahmedabad",
    "nameHi": "अहमदाबाद",
    "talukas": [
      {
        "nameGu": "અમદાવાદ શહેર",
        "nameEn": "Ahmedabad City",
        "nameHi": "अहमदाबाद शहर"
      },
      {
        "nameGu": "દસક્રોઇ",
        "nameEn": "Daskroi",
        "nameHi": "दसक्रोइ"
      },
      {
        "nameGu": "સાણંદ",
        "nameEn": "Sanand",
        "nameHi": "साणंद"
      },
      {
        "nameGu": "ધોળકા",
        "nameEn": "Dholka",
        "nameHi": "धोळका"
      },
      {
        "nameGu": "બાવળા",
        "nameEn": "Bavla",
        "nameHi": "बावळा"
      },
      {
        "nameGu": "માંડલ",
        "nameEn": "Mandal",
        "nameHi": "मांडल"
      },
      {
        "nameGu": "દેત્રોજ-રામપુરા",
        "nameEn": "Detroj-Rampura",
        "nameHi": "देतरोज-रामपुरा"
      },
      {
        "nameGu": "વિરમગામ",
        "nameEn": "Viramgam",
        "nameHi": "विरमगाम"
      }
    ]
  },
  {
    "nameGu": "સુરેન્દ્રનગર",
    "nameEn": "Surendranagar",
    "nameHi": "सुरेंद्रनगर",
    "talukas": [
      {
        "nameGu": "સુરેન્દ્રનગર",
        "nameEn": "Surendranagar",
        "nameHi": "सुरेन्द्रनगर"
      },
      {
        "nameGu": "ચોટીલા",
        "nameEn": "Chotila",
        "nameHi": "चोटीला"
      },
      {
        "nameGu": "ચુડા",
        "nameEn": "Chuda",
        "nameHi": "चुडा"
      },
      {
        "nameGu": "દસાડા",
        "nameEn": "Dasada",
        "nameHi": "दसाडा"
      },
      {
        "nameGu": "ધ્રાંગધ્રા",
        "nameEn": "Dhrangadhra",
        "nameHi": "ध्रांगध्रा"
      },
      {
        "nameGu": "લખતર",
        "nameEn": "Lakhtar",
        "nameHi": "लखतर"
      },
      {
        "nameGu": "લીંબડી",
        "nameEn": "Limbdi",
        "nameHi": "लींबडी"
      },
      {
        "nameGu": "મૂળી",
        "nameEn": "Muli",
        "nameHi": "मूळी"
      },
      {
        "nameGu": "સાયલા",
        "nameEn": "Sayla",
        "nameHi": "सायला"
      },
      {
        "nameGu": "થાનગઢ",
        "nameEn": "Thangadh",
        "nameHi": "थानगढ"
      },
      {
        "nameGu": "વઢવાણ",
        "nameEn": "Wadhwan",
        "nameHi": "वढवाण"
      }
    ]
  },
  {
    "nameGu": "મોરબી",
    "nameEn": "Morbi",
    "nameHi": "मोरबी",
    "talukas": [
      {
        "nameGu": "મોરબી",
        "nameEn": "Morbi",
        "nameHi": "मोरबी"
      },
      {
        "nameGu": "હળવદ",
        "nameEn": "Halvad",
        "nameHi": "हळवद"
      },
      {
        "nameGu": "માળિયા",
        "nameEn": "Maliya",
        "nameHi": "माळिया"
      },
      {
        "nameGu": "ટંકારા",
        "nameEn": "Tankara",
        "nameHi": "टंकारा"
      },
      {
        "nameGu": "વાંકાનેર",
        "nameEn": "Wankaner",
        "nameHi": "वांकानेर"
      }
    ]
  },
  {
    "nameGu": "રાજકોટ",
    "nameEn": "Rajkot",
    "nameHi": "राजकोट",
    "talukas": [
      {
        "nameGu": "રાજકોટ",
        "nameEn": "Rajkot",
        "nameHi": "राजकोट"
      },
      {
        "nameGu": "ધોરાજી",
        "nameEn": "Dhoraji",
        "nameHi": "धोराजी"
      },
      {
        "nameGu": "ગોંડલ",
        "nameEn": "Gondal",
        "nameHi": "गोंडल"
      },
      {
        "nameGu": "જામકંડોરણા",
        "nameEn": "Jamkandorna",
        "nameHi": "जामकंडोरणा"
      },
      {
        "nameGu": "જસદણ",
        "nameEn": "Jasdan",
        "nameHi": "जसदण"
      },
      {
        "nameGu": "જેતપુર",
        "nameEn": "Jetpur",
        "nameHi": "जेतपुर"
      },
      {
        "nameGu": "કોટડા સાંગાણી",
        "nameEn": "Kotda Sangani",
        "nameHi": "कोटड़ा सांगाणी"
      },
      {
        "nameGu": "લોધિકા",
        "nameEn": "Lodhika",
        "nameHi": "लोधिका"
      },
      {
        "nameGu": "પડધરી",
        "nameEn": "Paddhari",
        "nameHi": "पडधरी"
      },
      {
        "nameGu": "ઉપલેટા",
        "nameEn": "Upleta",
        "nameHi": "उपलेटा"
      },
      {
        "nameGu": "વિંછિયા",
        "nameEn": "Vinchhiya",
        "nameHi": "विंछिया"
      }
    ]
  },
  {
    "nameGu": "જામનગર",
    "nameEn": "Jamnagar",
    "nameHi": "जामनगर",
    "talukas": [
      {
        "nameGu": "જામનગર",
        "nameEn": "Jamnagar",
        "nameHi": "जामनगर"
      },
      {
        "nameGu": "ધ્રોલ",
        "nameEn": "Dhrol",
        "nameHi": "ध्रोल"
      },
      {
        "nameGu": "જોડિયા",
        "nameEn": "Jodiya",
        "nameHi": "जोडिया"
      },
      {
        "nameGu": "કાલાવડ",
        "nameEn": "Kalavad",
        "nameHi": "कालावड"
      },
      {
        "nameGu": "લાલપુર",
        "nameEn": "Lalpur",
        "nameHi": "लालपुर"
      },
      {
        "nameGu": "જામજોધપુર",
        "nameEn": "Jamjodhpur",
        "nameHi": "जामजोधपुर"
      }
    ]
  },
  {
    "nameGu": "દેવભૂમિ દ્વારકા",
    "nameEn": "Devbhumi Dwarka",
    "nameHi": "देवभूमि द्वारका",
    "talukas": [
      {
        "nameGu": "દ્વારકા",
        "nameEn": "Dwarka",
        "nameHi": "द्वारका"
      },
      {
        "nameGu": "કલ્યાણપુર",
        "nameEn": "Kalyanpur",
        "nameHi": "कल्याणपुर"
      },
      {
        "nameGu": "ખંભાળિયા",
        "nameEn": "Khambhalia",
        "nameHi": "खंभाळिया"
      },
      {
        "nameGu": "ભાણવડ",
        "nameEn": "Bhanvad",
        "nameHi": "भाणवड"
      }
    ]
  },
  {
    "nameGu": "પોરબંદર",
    "nameEn": "Porbandar",
    "nameHi": "पोरबंदर",
    "talukas": [
      {
        "nameGu": "પોરબંદર",
        "nameEn": "Porbandar",
        "nameHi": "पोरबंदर"
      },
      {
        "nameGu": "કુતિયાણા",
        "nameEn": "Kutiyana",
        "nameHi": "कुतियाणा"
      },
      {
        "nameGu": "રાણાવાવ",
        "nameEn": "Ranavav",
        "nameHi": "राणावाव"
      }
    ]
  },
  {
    "nameGu": "જુનાગઢ",
    "nameEn": "Junagadh",
    "nameHi": "जूनागढ़",
    "talukas": [
      {
        "nameGu": "જુનાગઢ",
        "nameEn": "Junagadh",
        "nameHi": "जुनागढ"
      },
      {
        "nameGu": "ભેસાણ",
        "nameEn": "Bhesan",
        "nameHi": "भेसाण"
      },
      {
        "nameGu": "કેશોદ",
        "nameEn": "Keshod",
        "nameHi": "केशोद"
      },
      {
        "nameGu": "માલિયા",
        "nameEn": "Malia",
        "nameHi": "मालिया"
      },
      {
        "nameGu": "માણાવદર",
        "nameEn": "Manavadar",
        "nameHi": "माणावदर"
      },
      {
        "nameGu": "માંગરોળ",
        "nameEn": "Mangrol",
        "nameHi": "मांगरोळ"
      },
      {
        "nameGu": "મેંદરડા",
        "nameEn": "Mendarda",
        "nameHi": "मेंदरडा"
      },
      {
        "nameGu": "વંથલી",
        "nameEn": "Vanthali",
        "nameHi": "वंथली"
      },
      {
        "nameGu": "વિસાવદર",
        "nameEn": "Visavadar",
        "nameHi": "विसावदर"
      }
    ]
  },
  {
    "nameGu": "ગીર સોમનાથ",
    "nameEn": "Gir Somnath",
    "nameHi": "गीर सोमनाथ",
    "talukas": [
      {
        "nameGu": "વેરાવળ",
        "nameEn": "Veraval",
        "nameHi": "वेरावळ"
      },
      {
        "nameGu": "ગીર ગઢડા",
        "nameEn": "Gir Gadhada",
        "nameHi": "गीर गढ़डा"
      },
      {
        "nameGu": "કોડીનાર",
        "nameEn": "Kodinar",
        "nameHi": "कोडीनार"
      },
      {
        "nameGu": "સુત્રાપાડા",
        "nameEn": "Sutrapada",
        "nameHi": "सुत्रापाडा"
      },
      {
        "nameGu": "તાલાલા",
        "nameEn": "Talala",
        "nameHi": "तालाला"
      },
      {
        "nameGu": "ઉના",
        "nameEn": "Una",
        "nameHi": "उना"
      }
    ]
  },
  {
    "nameGu": "અમરેલી",
    "nameEn": "Amreli",
    "nameHi": "अमरेली",
    "talukas": [
      {
        "nameGu": "અમરેલી",
        "nameEn": "Amreli",
        "nameHi": "अमरेली"
      },
      {
        "nameGu": "બાબરા",
        "nameEn": "Babra",
        "nameHi": "बाबरा"
      },
      {
        "nameGu": "ધારી",
        "nameEn": "Dhari",
        "nameHi": "धारी"
      },
      {
        "nameGu": "જાફરાબાદ",
        "nameEn": "Jafrabad",
        "nameHi": "जाफराबाद"
      },
      {
        "nameGu": "ખાંભા",
        "nameEn": "Khambha",
        "nameHi": "खांभा"
      },
      {
        "nameGu": "કુંકાવાવ વડિયા",
        "nameEn": "Kunkavav Vadia",
        "nameHi": "कुंकावाव वडिया"
      },
      {
        "nameGu": "લાઠી",
        "nameEn": "Lathi",
        "nameHi": "लाठी"
      },
      {
        "nameGu": "લીલીયા",
        "nameEn": "Lilia",
        "nameHi": "लीलीया"
      },
      {
        "nameGu": "રાજુલા",
        "nameEn": "Rajula",
        "nameHi": "राजुला"
      },
      {
        "nameGu": "સાવરકુંડલા",
        "nameEn": "Savarkundla",
        "nameHi": "सावरकुंडला"
      },
      {
        "nameGu": "બગસરા",
        "nameEn": "Bagasara",
        "nameHi": "बगसरा"
      }
    ]
  },
  {
    "nameGu": "ભાવનગર",
    "nameEn": "Bhavnagar",
    "nameHi": "भावनगर",
    "talukas": [
      {
        "nameGu": "ભાવનગર",
        "nameEn": "Bhavnagar",
        "nameHi": "भावनगर"
      },
      {
        "nameGu": "ગરિયાધાર",
        "nameEn": "Gariadhar",
        "nameHi": "गरियाधार"
      },
      {
        "nameGu": "ઘોઘા",
        "nameEn": "Ghogha",
        "nameHi": "घोघा"
      },
      {
        "nameGu": "જેસર",
        "nameEn": "Jesar",
        "nameHi": "जेसर"
      },
      {
        "nameGu": "મહુવા",
        "nameEn": "Mahuva",
        "nameHi": "महुवा"
      },
      {
        "nameGu": "પાલિતાણા",
        "nameEn": "Palitana",
        "nameHi": "पालिताणा"
      },
      {
        "nameGu": "સિહોર",
        "nameEn": "Sihor",
        "nameHi": "सिहोर"
      },
      {
        "nameGu": "તળાજા",
        "nameEn": "Talaja",
        "nameHi": "तळाजा"
      },
      {
        "nameGu": "ઉમરાળા",
        "nameEn": "Umrala",
        "nameHi": "उमराळा"
      },
      {
        "nameGu": "વલ્લભીપુર",
        "nameEn": "Vallabhipur",
        "nameHi": "वल्लभीपुर"
      }
    ]
  },
  {
    "nameGu": "બોટાદ",
    "nameEn": "Botad",
    "nameHi": "बोटाद",
    "talukas": [
      {
        "nameGu": "બોટાદ",
        "nameEn": "Botad",
        "nameHi": "बोटाद"
      },
      {
        "nameGu": "બરવાળા",
        "nameEn": "Barwala",
        "nameHi": "बरवाळा"
      },
      {
        "nameGu": "ગઢડા",
        "nameEn": "Gadhada",
        "nameHi": "गढडा"
      },
      {
        "nameGu": "રાણપુર",
        "nameEn": "Ranpur",
        "nameHi": "राणपुर"
      }
    ]
  },
  {
    "nameGu": "આણંદ",
    "nameEn": "Anand",
    "nameHi": "आणंद",
    "talukas": [
      {
        "nameGu": "આણંદ",
        "nameEn": "Anand",
        "nameHi": "आणंद"
      },
      {
        "nameGu": "અંકલાવ",
        "nameEn": "Anklav",
        "nameHi": "अंकलाव"
      },
      {
        "nameGu": "બોરસદ",
        "nameEn": "Borsad",
        "nameHi": "बोरसद"
      },
      {
        "nameGu": "ખંભાત",
        "nameEn": "Khambhat",
        "nameHi": "खंभात"
      },
      {
        "nameGu": "પેટલાદ",
        "nameEn": "Petlad",
        "nameHi": "पेटलाद"
      },
      {
        "nameGu": "સોજિત્રા",
        "nameEn": "Sojitra",
        "nameHi": "सोजित्रा"
      },
      {
        "nameGu": "તારાપુર",
        "nameEn": "Tarapur",
        "nameHi": "तारापुर"
      },
      {
        "nameGu": "ઉમરેઠ",
        "nameEn": "Umreth",
        "nameHi": "उमरेठ"
      }
    ]
  },
  {
    "nameGu": "ખેડા",
    "nameEn": "Kheda",
    "nameHi": "खेड़ा",
    "talukas": [
      {
        "nameGu": "નડિયાદ",
        "nameEn": "Nadiad",
        "nameHi": "नडियाद"
      },
      {
        "nameGu": "ગલતેશ્વર",
        "nameEn": "Galteshwar",
        "nameHi": "गलतेश्वर"
      },
      {
        "nameGu": "કપડવંજ",
        "nameEn": "Kapadvanj",
        "nameHi": "कपडवंज"
      },
      {
        "nameGu": "કઠલાલ",
        "nameEn": "Kathlal",
        "nameHi": "कठलाल"
      },
      {
        "nameGu": "મહુધા",
        "nameEn": "Mahudha",
        "nameHi": "महुधा"
      },
      {
        "nameGu": "માતર",
        "nameEn": "Matar",
        "nameHi": "मातर"
      },
      {
        "nameGu": "મહેમદાવાદ",
        "nameEn": "Mehmedabad",
        "nameHi": "महेमदावाद"
      },
      {
        "nameGu": "થાસરા",
        "nameEn": "Thasra",
        "nameHi": "थासरा"
      },
      {
        "nameGu": "વાસો",
        "nameEn": "Vaso",
        "nameHi": "वासो"
      }
    ]
  },
  {
    "nameGu": "મહીસાગર",
    "nameEn": "Mahisagar",
    "nameHi": "महीसागर",
    "talukas": [
      {
        "nameGu": "લુણાવાડા",
        "nameEn": "Lunawada",
        "nameHi": "लुणावाडा"
      },
      {
        "nameGu": "બાલાસિનોર",
        "nameEn": "Balasinor",
        "nameHi": "बालासिनोर"
      },
      {
        "nameGu": "કડાણા",
        "nameEn": "Kadana",
        "nameHi": "कडाणा"
      },
      {
        "nameGu": "ખાનપુર",
        "nameEn": "Khanpur",
        "nameHi": "खानपुर"
      },
      {
        "nameGu": "વિરપુર",
        "nameEn": "Virpur",
        "nameHi": "विरपुर"
      },
      {
        "nameGu": "સંતરામપુર",
        "nameEn": "Santrampur",
        "nameHi": "संतरामपुर"
      }
    ]
  },
  {
    "nameGu": "પંચમહાલ",
    "nameEn": "Panchmahal",
    "nameHi": "पंचमहाल",
    "talukas": [
      {
        "nameGu": "ગોધરા",
        "nameEn": "Godhra",
        "nameHi": "गोधरा"
      },
      {
        "nameGu": "ઘોઘંબા",
        "nameEn": "Ghoghamba",
        "nameHi": "घोघंबा"
      },
      {
        "nameGu": "હાલોલ",
        "nameEn": "Halol",
        "nameHi": "हालोल"
      },
      {
        "nameGu": "જાંબુઘોડા",
        "nameEn": "Jambughoda",
        "nameHi": "जांबुघोडा"
      },
      {
        "nameGu": "કલોલ",
        "nameEn": "Kalol",
        "nameHi": "कलोल"
      },
      {
        "nameGu": "મોરવા હાડફ",
        "nameEn": "Morva Hadaf",
        "nameHi": "मोरवा हाडफ"
      },
      {
        "nameGu": "શહેરા",
        "nameEn": "Shehera",
        "nameHi": "शहेरा"
      }
    ]
  },
  {
    "nameGu": "દાહોદ",
    "nameEn": "Dahod",
    "nameHi": "दाहोद",
    "talukas": [
      {
        "nameGu": "દાહોદ",
        "nameEn": "Dahod",
        "nameHi": "दाहोद"
      },
      {
        "nameGu": "દેવગઢ બારિયા",
        "nameEn": "Devgadh Baria",
        "nameHi": "देवगढ़ बारिया"
      },
      {
        "nameGu": "ધાનપુર",
        "nameEn": "Dhanpur",
        "nameHi": "धानपुर"
      },
      {
        "nameGu": "ફતેપુરા",
        "nameEn": "Fatepura",
        "nameHi": "फतेपुरा"
      },
      {
        "nameGu": "ગરબાડા",
        "nameEn": "Garbada",
        "nameHi": "गरबाडा"
      },
      {
        "nameGu": "ઝાલોદ",
        "nameEn": "Jhalod",
        "nameHi": "झालोद"
      },
      {
        "nameGu": "લીમખેડા",
        "nameEn": "Limkheda",
        "nameHi": "लीमखेडा"
      },
      {
        "nameGu": "સાંજેલી",
        "nameEn": "Sanjeli",
        "nameHi": "सांजेली"
      },
      {
        "nameGu": "સિંગવડ",
        "nameEn": "Singvad",
        "nameHi": "सिंगवड"
      }
    ]
  },
  {
    "nameGu": "વડોદરા",
    "nameEn": "Vadodara",
    "nameHi": "वडोदरा",
    "talukas": [
      {
        "nameGu": "વડોદરા",
        "nameEn": "Vadodara",
        "nameHi": "वडोदरा"
      },
      {
        "nameGu": "ડભોઇ",
        "nameEn": "Dabhoi",
        "nameHi": "डभोइ"
      },
      {
        "nameGu": "દેસર",
        "nameEn": "Desar",
        "nameHi": "देसर"
      },
      {
        "nameGu": "કરજણ",
        "nameEn": "Karjan",
        "nameHi": "करजण"
      },
      {
        "nameGu": "પાદરા",
        "nameEn": "Padra",
        "nameHi": "पादरा"
      },
      {
        "nameGu": "સાવલી",
        "nameEn": "Savli",
        "nameHi": "सावली"
      },
      {
        "nameGu": "સિનોર",
        "nameEn": "Sinor",
        "nameHi": "सिनोर"
      },
      {
        "nameGu": "વાઘોડિયા",
        "nameEn": "Waghodia",
        "nameHi": "वाघोडिया"
      }
    ]
  },
  {
    "nameGu": "છોટા ઉદેપુર",
    "nameEn": "Chhota Udepur",
    "nameHi": "छोटा उदेपुर",
    "talukas": [
      {
        "nameGu": "છોટા ઉદેપુર",
        "nameEn": "Chhota Udepur",
        "nameHi": "छोटा उदेपुर"
      },
      {
        "nameGu": "બોડેલી",
        "nameEn": "Bodeli",
        "nameHi": "बोडेली"
      },
      {
        "nameGu": "જેતપુર પાવી",
        "nameEn": "Jetpur Pavi",
        "nameHi": "जेतपुर पावी"
      },
      {
        "nameGu": "કવંત",
        "nameEn": "Kavant",
        "nameHi": "कवंत"
      },
      {
        "nameGu": "નસવાડી",
        "nameEn": "Naswadi",
        "nameHi": "नसवाडी"
      },
      {
        "nameGu": "સંખેડા",
        "nameEn": "Sankheda",
        "nameHi": "संखेडा"
      }
    ]
  },
  {
    "nameGu": "નર્મદા",
    "nameEn": "Narmada",
    "nameHi": "नर्मदा",
    "talukas": [
      {
        "nameGu": "રાજપીપળા",
        "nameEn": "Rajpipla",
        "nameHi": "राजपीपळा"
      },
      {
        "nameGu": "દેડિયાપાડા",
        "nameEn": "Dediapada",
        "nameHi": "देडियापाडा"
      },
      {
        "nameGu": "ગરુડેશ્વર",
        "nameEn": "Garudeshwar",
        "nameHi": "गरुडेश्वर"
      },
      {
        "nameGu": "નાંદોદ",
        "nameEn": "Nandod",
        "nameHi": "नांदोद"
      },
      {
        "nameGu": "સાગબારા",
        "nameEn": "Sagbara",
        "nameHi": "सागबारा"
      },
      {
        "nameGu": "તિલકવાડા",
        "nameEn": "Tilakwada",
        "nameHi": "तिलकवाडा"
      }
    ]
  },
  {
    "nameGu": "ભરૂચ",
    "nameEn": "Bharuch",
    "nameHi": "भरूच",
    "talukas": [
      {
        "nameGu": "ભરૂચ",
        "nameEn": "Bharuch",
        "nameHi": "भरूच"
      },
      {
        "nameGu": "આમોદ",
        "nameEn": "Amod",
        "nameHi": "आमोद"
      },
      {
        "nameGu": "અંકલેશ્વર",
        "nameEn": "Ankleshwar",
        "nameHi": "अंकलेश्वर"
      },
      {
        "nameGu": "હાંસોટ",
        "nameEn": "Hansot",
        "nameHi": "हांसोट"
      },
      {
        "nameGu": "જંબુસર",
        "nameEn": "Jambusar",
        "nameHi": "जंबुसर"
      },
      {
        "nameGu": "ઝગડિયા",
        "nameEn": "Jhagadia",
        "nameHi": "झगडिया"
      },
      {
        "nameGu": "વાગરા",
        "nameEn": "Vagra",
        "nameHi": "वागरा"
      },
      {
        "nameGu": "વાલિયા",
        "nameEn": "Valia",
        "nameHi": "वालिया"
      }
    ]
  },
  {
    "nameGu": "સુરત",
    "nameEn": "Surat",
    "nameHi": "सूरत",
    "talukas": [
      {
        "nameGu": "સુરત શહેર",
        "nameEn": "Surat City",
        "nameHi": "सूरत शहर"
      },
      {
        "nameGu": "બારડોલી",
        "nameEn": "Bardoli",
        "nameHi": "बारडोली"
      },
      {
        "nameGu": "ચોર્યાસી",
        "nameEn": "Choryasi",
        "nameHi": "चोर्यासी"
      },
      {
        "nameGu": "કામરેજ",
        "nameEn": "Kamrej",
        "nameHi": "कामरेज"
      },
      {
        "nameGu": "મહુવા",
        "nameEn": "Mahuva",
        "nameHi": "महुवा"
      },
      {
        "nameGu": "માંડવી",
        "nameEn": "Mandvi",
        "nameHi": "मांडवी"
      },
      {
        "nameGu": "માંગરોળ",
        "nameEn": "Mangrol",
        "nameHi": "मांगरोळ"
      },
      {
        "nameGu": "ઓલપાડ",
        "nameEn": "Olpad",
        "nameHi": "ओलपाड"
      },
      {
        "nameGu": "પલસાણા",
        "nameEn": "Palsana",
        "nameHi": "पलसाणा"
      },
      {
        "nameGu": "ઉમરપાડા",
        "nameEn": "Umarpada",
        "nameHi": "उमरपाडा"
      }
    ]
  },
  {
    "nameGu": "તાપી",
    "nameEn": "Tapi",
    "nameHi": "तापी",
    "talukas": [
      {
        "nameGu": "વ્યારા",
        "nameEn": "Vyara",
        "nameHi": "व्यारा"
      },
      {
        "nameGu": "ડોલવણ",
        "nameEn": "Dolvan",
        "nameHi": "डोलवण"
      },
      {
        "nameGu": "કુકરમુંડા",
        "nameEn": "Kukarmunda",
        "nameHi": "कुकरमुंडा"
      },
      {
        "nameGu": "નિઝર",
        "nameEn": "Nizar",
        "nameHi": "निझर"
      },
      {
        "nameGu": "સોનગઢ",
        "nameEn": "Songadh",
        "nameHi": "सोनगढ"
      },
      {
        "nameGu": "ઉચ્છલ",
        "nameEn": "Uchchhal",
        "nameHi": "उच्छल"
      },
      {
        "nameGu": "વલોડ",
        "nameEn": "Valod",
        "nameHi": "वलोड"
      }
    ]
  },
  {
    "nameGu": "નવસારી",
    "nameEn": "Navsari",
    "nameHi": "नवसारी",
    "talukas": [
      {
        "nameGu": "નવસારી",
        "nameEn": "Navsari",
        "nameHi": "नवसारी"
      },
      {
        "nameGu": "વાંસદા",
        "nameEn": "Vansda",
        "nameHi": "वांसदा"
      },
      {
        "nameGu": "ચીખલી",
        "nameEn": "Chikhli",
        "nameHi": "चीखली"
      },
      {
        "nameGu": "ગણદેવી",
        "nameEn": "Gandevi",
        "nameHi": "गणदेवी"
      },
      {
        "nameGu": "જલાલપોર",
        "nameEn": "Jalalpore",
        "nameHi": "जलालपोर"
      },
      {
        "nameGu": "ખેરગામ",
        "nameEn": "Khergam",
        "nameHi": "खेरगाम"
      }
    ]
  },
  {
    "nameGu": "વલસાડ",
    "nameEn": "Valsad",
    "nameHi": "वलसाड",
    "talukas": [
      {
        "nameGu": "વલસાડ",
        "nameEn": "Valsad",
        "nameHi": "वलसाड"
      },
      {
        "nameGu": "ધરમપુર",
        "nameEn": "Dharampur",
        "nameHi": "धरमपुर"
      },
      {
        "nameGu": "કપરાડા",
        "nameEn": "Kaprada",
        "nameHi": "कपराडा"
      },
      {
        "nameGu": "પારડી",
        "nameEn": "Pardi",
        "nameHi": "पारडी"
      },
      {
        "nameGu": "ઉમરગામ",
        "nameEn": "Umbergaon",
        "nameHi": "उमरगाम"
      },
      {
        "nameGu": "વાપી",
        "nameEn": "Vapi",
        "nameHi": "वापी"
      }
    ]
  },
  {
    "nameGu": "ડાંગ",
    "nameEn": "Dang",
    "nameHi": "डांग",
    "talukas": [
      {
        "nameGu": "આહવા",
        "nameEn": "Ahwa",
        "nameHi": "आहवा"
      },
      {
        "nameGu": "સુબીર",
        "nameEn": "Subir",
        "nameHi": "सुबीर"
      },
      {
        "nameGu": "વાઘઇ",
        "nameEn": "Waghai",
        "nameHi": "वाघइ"
      }
    ]
  }
];

export function talukasForDistrict(districtName) {
  const key = String(districtName || "").trim();
  const district = GUJARAT_DISTRICTS.find(
    (item) => item.nameGu === key || item.nameEn === key || item.nameHi === key || item.name === key
  );
  return (district?.talukas || []).map((t) => (typeof t === "string" ? t : t.nameGu));
}

export function findDistrict(districtName) {
  const key = String(districtName || "").trim();
  return (
    GUJARAT_DISTRICTS.find(
      (item) => item.nameGu === key || item.nameEn === key || item.nameHi === key || item.name === key
    ) || null
  );
}

/** Find parent district for a taluka name stored in any language. */
export function findDistrictForTaluka(talukaName) {
  const key = String(talukaName || "").trim();
  if (!key) return null;
  for (const district of GUJARAT_DISTRICTS) {
    const hit = (district.talukas || []).find(
      (item) =>
        item === key ||
        item.nameGu === key ||
        item.nameEn === key ||
        item.nameHi === key
    );
    if (hit) return district;
  }
  return null;
}

/** Canonical Gujarati name for a district/taluka value in any language. */
export function canonicalPlaceName(name, { districtName } = {}) {
  const raw = String(name || "").trim();
  if (!raw) return "";
  if (!districtName) {
    const district = findDistrict(raw);
    if (district) return district.nameGu;
  }
  const parent = findDistrict(districtName) || findDistrictForTaluka(raw);
  const taluka = (parent?.talukas || []).find(
    (item) => item.nameGu === raw || item.nameEn === raw || item.nameHi === raw || item === raw
  );
  if (taluka) return typeof taluka === "string" ? taluka : taluka.nameGu;
  const fallbackDistrict = findDistrictForTaluka(raw);
  const fallbackTaluka = (fallbackDistrict?.talukas || []).find(
    (item) => item.nameGu === raw || item.nameEn === raw || item.nameHi === raw
  );
  if (fallbackTaluka) return fallbackTaluka.nameGu;
  return raw;
}
